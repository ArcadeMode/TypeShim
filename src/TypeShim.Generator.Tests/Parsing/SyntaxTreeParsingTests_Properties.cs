...
internal class SyntaxTreeParsingTests_Properties
{
    ...
    [Test]
    public void ClassInfoBuilder_PublicInstanceProperty_Parses()
    {
        SyntaxTree syntaxTree = CSharpSyntaxTree.ParseText("""
            using System;
            namespace N1;
        ...
        PropertyInfo propertyInfo = classInfo.Properties.First();
        Assert.That(propertyInfo.Name, Is.EqualTo("P1"));
        Assert.That(propertyInfo.GetMethod.IsStatic, Is.False);
        Assert.That(propertyInfo.SetMethod?.IsStatic, Is.False);
    }

    [Test]
    public void ClassInfoBuilder_PublicRequiredInstanceProperty_Parses()
    {
        SyntaxTree syntaxTree = CSharpSyntaxTree.ParseText("""
            using System;
            namespace N1;
        ...
        Assert.That(propertyInfo.GetMethod.IsStatic, Is.False);
        Assert.That(propertyInfo.SetMethod?.IsStatic, Is.False);
        Assert.That(propertyInfo.InitMethod, Is.Null);

    }

    [Test]
    public void ClassInfoBuilder_PublicInstanceProperty_GetInit_Parses()
    {
        SyntaxTree syntaxTree = CSharpSyntaxTree.ParseText("""
            using System;
            namespace N1;
        ...
        Assert.That(propertyInfo.Name, Is.EqualTo("P1"));
        Assert.That(propertyInfo.GetMethod.IsStatic, Is.False);
        Assert.That(propertyInfo.SetMethod, Is.Null);
        Assert.That(propertyInfo.InitMethod, Is.Not.Null);
    }
